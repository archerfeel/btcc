...
pub struct Mnemonic(pub Vec<U11>);

pub type U11 = u16;

impl Mnemonic {
    pub fn from_phrase(phrase: &str) -> Result<Self, String> {
        let mut u11s = Vec::<U11>::with_capacity(12);
        for p in phrase.split(" ") {
            match find_en(p) {
                Some(u11) => u11s.push(u11),
                None => return Err(format!("Illegal mnemonic phrase {}", p)),
            }
        }
        Ok(Self(u11s))
    }

    pub fn new(u11s: Vec<U11>) -> Mnemonic {
        Self(u11s)
    }

    pub fn to_phrase(&self) -> String {
        self.0.iter().map(|u| get_en(*u)).collect::<Vec<String>>().join(" ")
    }
}

pub fn get_en(idx: U11) -> String {
    WORDS_EN[idx as usize].to_string()
}

pub fn find_en(phrase: &str) -> Option<U11> {
    find(phrase, &WORDS_EN, 0, 2047)
}

fn find(phrase: &str, words: &[&str], start: usize, end: usize) -> Option<U11> {
    let mid = (start + end) / 2;
    if phrase == words[mid] {
    ...
    if phrase < words[mid] {
        return find(phrase, &words, start, mid);
    }
    find(phrase, &words, mid + 1, end)
}

pub fn inspect_seed(mnemonic_phrase: &str, pass_phrase: &str) -> Vec<u8> {
    let mut seed = vec![0u8; SEED_LENGTH];
    let salt = format!("mnemonic{}", pass_phrase);
...
#[test]
pub fn test() {
    assert_eq!(None, find_en("boyyyyy"));
    assert_eq!(Some(2047), find_en("zoo"));
    assert_eq!(Some(2), find_en("able"));
    assert_eq!(Some(100), find_en(WORDS_EN[100]));
    assert_eq!("zoo", get_en(2047));
    ...
    let ans = memo.to_phrase();
    assert_eq!(phrase, &ans);
}
